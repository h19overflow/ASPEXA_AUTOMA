...
class ScanConfigContract(StrictBaseModel):
    ...
    approach: str = Field(default="standard", description="Scan intensity: quick, standard, thorough")
    custom_probes: List[str] = Field(default_factory=list, description="Custom probes to run")
    max_probes: int = Field(default=3, description="Maximum probes per agent")
    max_prompts_per_probe: int = Field(default=5, description="Maximum prompts per probe")
    # Rate limiting
