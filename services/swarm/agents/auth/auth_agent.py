"""
Auth Agent for authorization surface vulnerability scanning.

Purpose: Plan and execute BOLA/RBAC/privilege escalation scans
...
from .auth_probes import AUTH_PROBES, get_available_probes, get_probe_categories
...
class AuthAgent(BaseAgent):
    """Agent specializing in authorization and access control attacks.
    ...
    def system_prompt(self) -> str:
        return AUTH_SYSTEM_PROMPT.format(
    ...
    async def plan(self, recon_context: Dict[str, Any]) -> ProbePlan:
        ...
        user_message = f"""
        Analyze this target and select authorization surface probes:
        ...
        Steps:
        1. Use analyze_auth_infrastructure to assess auth-specific risks
        2. Use get_auth_probe_list to see available probes for the approach
        3. Select the most relevant probes with reasoning for each

        Focus on probes that test for:
        - Broken Object-Level Authorization (BOLA)
        - Role and permission bypass
        - Data leakage through authorization gaps
        """

        logger.info(f"AuthAgent planning with approach: {approach}")
