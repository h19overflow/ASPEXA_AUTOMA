...
from services.swarm.core.utils import log_scan_start, log_scan_complete, log_scan_error, log_performance_metric
...
def create_scanning_agent(agent_type: str, model_name: str = "google_genai:gemini-2.5-flash", use_structured_output: bool = True):
    """
    ...
    if use_structured_output:
        try:
            ...
            logger.debug("Configured structured output with ToolStrategy for %s", agent_type)
...
"""
        ...
        result = await agent_executor.ainvoke({"messages": [HumanMessage(content=input_message)]})
        ...
        for msg in messages:
            ...
            is_execute_scan_result = (
                ...
                getattr(msg, "name", None) == "execute_scan"
            ) or (
                hasattr(msg, "name") and msg.name == "execute_scan"
            )
                    ...
                    if content:
                        ...
                        if isinstance(tool_result, dict):
                            ...
                            if tool_result.get("success"):
                                ...
                                generations_used = tool_result.get("generations_used", 0)
                                report_path = tool_result.get("report_path")
                                scan_metadata = tool_result.get("metadata", {})

                                logger.info(f"Extracted scan results: {len(vulnerabilities)} vulnerabilities, "
                                          f"{len(probes_executed)} probes, report: {report_path}")
                            else:
                                # Scan failed - log the error
                                error = tool_result.get("error", "Unknown error")
                                logger.warning(f"execute_scan reported failure: {error}")
        ...
        log_performance_metric("agent_execution_time", duration, "seconds", scan_input.audit_id, agent_type)
        log_scan_complete(
            audit_id=scan_input.audit_id,
            agent_type=agent_type,
            duration=duration,
